...
def listar_agendamientos_filtros(estado=None, desde=None, hasta=None, responsable_id=None):
    ...
    try:
        query = """
            ...
            LEFT JOIN aspirantes c ON a.creador_id = c.id
            LEFT JOIN administradores d ON a.responsable_id = d.id
            WHERE 1=1
        """
        params = []
...
def listar_agendamientos_grupales():
    ...
    try:
        cur.execute("""
            ...
            LEFT JOIN aspirantes c ON ac.creador_id = c.id
            ORDER BY a.fecha_inicio DESC;
        """)
        resultados = cur.fetchall()
        columnas = [desc[0] for desc in cur.description]
        ...
        # Agrupar aspirantes por agendamiento
        agendamientos_dict = {}
        for fila in filas:

...
def actualizar_preevaluacion_perfil(aspirante_id: int, payload: dict):
...
def forzar_cambio_estado_por_id(aspirante_id: int, nuevo_id_estado: int):
    """
    Actualiza directamente el estado de un aspirante usando el ID numérico del estado.

    Args:
        aspirante_id (int): ID del aspirante (ej: 3236).
    ...
    try:
        with get_connection_context() as conn:
            with conn.cursor() as cur:
                ...
                if cur.rowcount > 0:
                    print(f"✅ [DB] Creador {aspirante_id} actualizado al estado ID {nuevo_id_estado}.")
                    return True
                else:
                    print(f"⚠️ [DB] No se encontró el creador ID {aspirante_id}.")
                    return False
...
# ):
#     """
#     - Actualiza en aspirantes_perfil: apariencia, engagement, calidad_contenido, eval_biografia, metadata_videos, eval_foto
...
#                     return {
#                         "status": "ok",
#                         "mensaje": "aspirantes_perfil actualizado. No hay variables en diagnostico_variable con categoria_id=1",
#                         "aspirante_id": aspirante_id,
...
#                         WHERE tvs.aspirante_id = %s
#                           AND tvs.variable_id = v.variable_id
#                     """ % ",".join(["(%s,%s)"] * len(pairs)),
#                     tuple([x for p in pairs for x in (p[0], p[1])] + [aspirante_id]))
#                     upsert_actualizadas = cur.rowcount
#
#                     # 5) Insert faltantes
#                     cur.execute("""
#                         INSERT INTO diagnostico_score_variable (aspirante_id, variable_id, score)
...
#                     tuple([aspirante_id] + [x for p in pairs for x in (p[0], p[1])] + [aspirante_id]))
...
#         return {
#             "status": "ok",
#             "mensaje": "aspirantes_perfil actualizado + diagnostico_score_variable actualizado/insertado para variables categoria_id=1 (mapeadas por campo_db/nombre)",
#             "aspirante_id": aspirante_id,
#             "aspirantes_perfil_filas_afectadas": perfil_rows,
...
# )
# def obtener_cualitativo_aspirantes_perfil(
#     aspirante_id: int,
#     usuario_actual: dict = Depends(obtener_usuario_actual),
# ):
#     try:
...
#
#         if not row:
#             raise HTTPException(status_code=404, detail="aspirantes_perfil no encontrado para ese aspirante_id")
#
...
#         raise HTTPException(
#             status_code=500,
#             detail=f"Error al obtener cualitativo de aspirantes_perfil: {str(e)}"
#         )
    ...
    with get_connection_context() as conn:
        with conn.cursor() as cur:
            ...
            if not row:
                return {
                    "status": "error",
                    "msg": "aspirantes_perfil no encontrado",
                    "aspirante_id": aspirante_id
                }
...
def recalcular_y_guardar_pre_resumen_v2(
    ...
    with get_connection_context() as conn:
        with conn.cursor() as cur:
            ...
            if not row:
                return {"status": "error", "msg": "aspirantes_perfil no encontrado"}
...
def _obtener_pre_resumen_guardado(aspirante_id: int) -> dict:
    with get_connection_context() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT
...
@router.get(
    "/api/aspirantes_perfil/{aspirante_id}/pre_resumen",
    tags=["Resumen Pre-Evaluación"],
...
def obtener_pre_resumen(
    ...
    resultado = _obtener_pre_resumen_guardado(aspirante_id)
    if resultado.get("status") != "ok":
        raise HTTPException(status_code=404, detail="Perfil no encontrado")

    # ✅ 2) Diagnóstico (usa DB; tu función ya debe parsear JSON internamente)
    try:
        # Si tu diagnostico usa obtener_datos_mejoras_aspirantes_perfil y lee DB,
        # perfecto: no recalcula puntajes.
        diagnostico = diagnostico_aspirantes_perfil_pre(aspirante_id, puntajes_calculados=resultado)
    except Exception:
        diagnostico = "-"
...
@router.get("/api/aspirantes_perfil/{aspirante_id}/pre_resumenV1",
         tags=["Resumen Pre-Evaluación"],
         response_model=ResumenEvaluacionOutput)
def obtener_pre_resumenV1(aspirante_id: int, usuario_actual: dict = Depends(obtener_usuario_actual)):
...
@router.post("/api/aspirantes/{aspirante_id}/talento-score")
def guardar_talento_score(aspirante_id: int, data: TalentoScoreCreate):
    TENANT = current_tenant.get()
    if not TENANT:
        raise HTTPException(status_code=400, detail="Tenant no disponible")
    ...
    with get_connection_context() as conn:
        ...
        cur.execute("""
            ...
            (aspirante_id, variable_id, score)
            VALUES (%s, %s, %s)
            ON CONFLICT DO NOTHING
        """, (aspirante_id, data.variable_id, data.score))
...
@router.post("/api/aspirantes/{aspirante_id}/evaluar/{modelo_id}")
def evaluar_creador(aspirante_id: int, modelo_id: int):

    TENANT = current_tenant.get()
    if not TENANT:
        raise HTTPException(status_code=400, detail="Tenant no disponible")

    resultado = calcular_evaluacion(aspirante_id, modelo_id)
...
@router.get("/api/aspirantes/{aspirante_id}/resultados",
            response_model=List[EvaluacionResultadoOut])
def listar_resultados(aspirante_id: int):
...
def calcular_evaluacion(aspirante_id: int, modelo_id: int):
    ...
    with get_connection_context() as conn:
        ...
        for cat_id, cat_nombre, cat_peso in categorias:
            ...
            for var_id, campo_db, peso_variable, tipo in variables:
                ...
                # 🔹 CUANTITATIVA (desde aspirantes_perfil)

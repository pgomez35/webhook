...
from utils_aspirantes import registrar_cambio_estado, registrar_cambio_estado_con_cursor
...
def crear_o_actualizar_creador_desde_aspirante(
    ...
    """
    Crea o actualiza el registro en creadores a partir del aspirante incorporado.
    Evita duplicados usando aspirante_id.
    """
    ...
    if not aspirante:
        raise HTTPException(
            status_code=404,
            detail="No se encontró el aspirante para crear el creador"
        )
    ...
    """, (
        aspirante[1],  # usuario
        aspirante[0],  # aspirante_id
        aspirante[2],  # nickname
        aspirante[3],  # nombre_real
        aspirante[4],  # email
        aspirante[5],  # telefono
        aspirante[6],  # whatsapp
        aspirante[7],  # foto_url
        manager_id,
        fecha_incorporacion
    ))

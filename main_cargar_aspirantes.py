...
#     y ACTUALIZA/INSERTA aspirantes.fecha_solicitud cuando venga en el TXT.
#     """
#     conn = get_connection()
#     cur = conn.cursor()
#     resultados = []
...
#     nombre_hoja: str = Form(...),
#     str_key: str = Form(...),
#     txt_file: UploadFile = File(None),     # ⬅ puede venir el archivo
#     ruta_txt: str = Form(None),            # ⬅ o la ruta previamente subida
# ):
#     try:
#         logger.info(f"📥 Iniciando carga: hoja={nombre_hoja}, str_key={str_key}")
#
#         # 1) Resolver la ruta del TXT: o viene el archivo, o una ruta válida del servidor
#         if txt_file is not None:
#             if not (txt_file.filename or "").lower().endswith(".txt"):
#                 return {"status": "error", "mensaje": "El archivo TXT debe tener extensión .txt"}
#
...
#             logger.info(f"🗂️ TXT guardado en {ruta_txt_resuelta}")
#         else:
#             # Si no vino archivo, debe venir una ruta existente en el servidor
#             if not ruta_txt:
#                 return {"status": "error", "mensaje": "Falta el TXT: envía txt_file (archivo) o ruta_txt (servidor)."}
#             from pathlib import Path
#             if not Path(ruta_txt).exists():
#                 return {"status": "error", "mensaje": f"Ruta TXT no encontrada en el servidor: {ruta_txt}"}
#             ruta_txt_resuelta = ruta_txt
#
#         # 2) Extraer aspirantes
#         aspirantes = obtener_aspirantes_desde_hoja(str_key, nombre_hoja, ruta_txt_resuelta)
#         if not aspirantes:
#             logger.warning(f"⚠️ No se encontraron aspirantes en la hoja {nombre_hoja}")
...
# @router.post("/cargar_aspirantes_local")
# async def cargar_aspirantes_desde_archivo(
#     file: UploadFile = File(...),              # Excel (.xlsx/.xlsm) OBLIGATORIO
#     txt_file: UploadFile = File(...),          # TXT OBLIGATORIO ahora
#     nombre_hoja: str | None = Form(None),      # Nombre de hoja opcional
# ):
#     """
#     Carga aspirantes desde un Excel local y, opcionalmente, un TXT con métricas/nickname.
#     Estructura esperada del Excel (igual a workspace):
...
#             logger.info(f"✅ TXT parseado, usuarios encontrados: {len(info_por_usuario)}")
#         else:
#             logger.info("ℹ️ No se adjuntó TXT; se procesará solo el Excel.")
#
#         # ========= 2) Guardar Excel =========
#         fname = (file.filename or "").lower()
#         if not (fname.endswith(".xlsx") or fname.endswith(".xlsm")):
...
#             ws = wb[nombre_hoja]
#         else:
#             ws = wb[wb.sheetnames[0]]  # primera hoja, determinista
#         logger.info(f"📄 Hoja usada (local): {ws.title}")
#
#         # ========= 4) Detectar última fila por Columna B (usuarios) =========
#         col_B = 2
#         start_row = 4
#         consecutive_empties = 0
#         consecutive_empties_limit = 8

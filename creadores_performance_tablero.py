...
def _nombre_manager_visible(reporte: Dict[str, Any]) -> Optional[str]:
    """Preferir nombre_completo del admin; fallback al agente original del Backstage."""
    nombre = reporte.get("manager_nombre")
    if nombre and str(nombre).strip():
        return str(nombre).strip()
    agente = reporte.get("agente")
    if agente and str(agente).strip():
